import { DailyQuestProgress, MetaUpgradeShopItem, UIButton, UpgradeOption } from "../types";
...
interface StartPanelData {
  ...
  totalKills: number;
  debug: boolean;
  activeTab: "chest" | "home" | "weapon" | "mode";
  chestClaimed: boolean;
  freeBuffCharges: number;
  dna: number;
...
export class UIManager {
  ...
  drawSettlementPanel(
    ...
    width: number,
    height: number,
    info: { kills: number; time: number; reward: number; doubled: boolean }
  ): void {
  ...
  private drawButton(ctx: CanvasRenderingContext2D, button: UIButton): void {
