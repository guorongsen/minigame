...
export class DailyChallengeSystem {
  ...
  getDateKey(now = new Date()): string {
  ...
  getTodayChallenge(now = new Date()): DailyChallengeTemplate {
    const dateKey = this.getDateKey(now);
    return this.getChallengeByDateKey(dateKey);
  }

  getChallengeByDateKey(dateKey: string): DailyChallengeTemplate {
  ...
  private getDateSeed(dateKey: string): number {
