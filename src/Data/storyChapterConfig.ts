import { FragmentReward, StoryChapterConfig } from "../types";
import { baseWeaponIds } from "./weaponConfig";
...
const baseStoryChapterConfigList: StoryChapterConfig[] = Array.from({ length: chapterCount }, (_, index) => {
  const level = index + 1;
  return {
    id: `chapter_${level}`,
    level,
...
const calcRecommendedPower = (level: number): number => {
  const lv = Math.max(1, Math.floor(level));
  return Math.floor(120 + lv * lv * 4 + lv * 24);
};

const calcKillTarget = (level: number): number => {
  const lv = Math.max(1, Math.floor(level));
  return Math.floor(85 + lv * 10 + lv * lv * 0.45);
};

const calcBossTuning = (level: number): { hpMul: number; speedMul: number; damageMul: number } => {
...
const calcEnemyWeightMulById = (level: number): Record<string, number> => {
...
const calcFirstClearRewards = (level: number): FragmentReward[] => {
  if (baseWeaponIds.length <= 0) {
    return [];
  }

  const lv = Math.max(1, Math.floor(level));
  const mainIndex = (lv - 1) % baseWeaponIds.length;
  const subIndex = (mainIndex + 1) % baseWeaponIds.length;
  const rewards: FragmentReward[] = [
    {
      baseWeaponId: baseWeaponIds[mainIndex],
      amount: 10 + lv * 2
    },
    {
      baseWeaponId: baseWeaponIds[subIndex],
      amount: 6 + Math.floor(lv * 1.4)
    }
  ];

  if (lv >= 8) {
    const extraIndex = (mainIndex + 2) % baseWeaponIds.length;
    rewards.push({
      baseWeaponId: baseWeaponIds[extraIndex],
...
const calcChapterCombatScale = (level: number): {
...
export const storyChapterConfigList: StoryChapterConfig[] = baseStoryChapterConfigList.map((chapter) => ({
  ...chapter,
  ...calcChapterCombatScale(chapter.level),
  recommendedPower: calcRecommendedPower(chapter.level),
  starGoals: {
    killTarget: calcKillTarget(chapter.level)
  },
  firstClearFragmentRewards: calcFirstClearRewards(chapter.level),
  enemyWeightMulById: calcEnemyWeightMulById(chapter.level),
  bossTuning: calcBossTuning(chapter.level)
}));
