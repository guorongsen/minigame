...
export const evolutionConfigList: EvolutionConfig[] = [
  {
...
export const evolutionByFromWeapon: Record<string, EvolutionConfig[]> = evolutionConfigList.reduce((map, item) => {
  if (!map[item.fromWeaponId]) {
    map[item.fromWeaponId] = [];
  }
  map[item.fromWeaponId].push(item);
  return map;
}, {} as Record<string, EvolutionConfig[]>);
