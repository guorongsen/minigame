...
class UpgradeSystem {
    ...
    generateLevelUpOptions(player, hooks, count = 3, context) {
     ...
     */
    generateChestOptions(player, hooks, boosted = false, context) {
    ...
    generateChestMutationOptions(player, hooks, boosted = false, context) {
        ...
        const weaponRoll = this.pickUnique(weaponCandidates, weaponCount, {
            forceEvolutionAssist: !!(context === null || context === void 0 ? void 0 : context.forceEvolutionAssist)
        });
    ...
    generateExtraAdOption(player, hooks, existingIds) {
                        ...
                        option: {
                            ...
                            apply: () => {
                                if (player.levelWeapon(baseId)) {
                                    hooks.onFeedback(`${this.cfg.getWeapon(currentWeaponId).name} \u5347\u81f3${level + 1}\u7ea7`, this.cfg.getWeapon(currentWeaponId).color);
                                }
                            }
                        }
                    });
    ...
    buildCharacterMutationCandidates(player, hooks) {
        const candidates = [];
    ...
    getCharacterFusionSupportScore(player, geneId) {
        let score = 0;
    ...
    getElementEvolutionSupport(player, elementId) {
        let score = 0;
    ...
    getWeaponLevelEvolutionSupport(player, baseWeaponId, nextLevel) {
    ...
    getDefinitionWeightMultiplier(player, def) {
    ...
    applyDefinition(def, player, hooks) {
    ...
    pickUnique(candidates, count, context) {
        const pool = [...candidates];
        const selected = [];
        while (selected.length < count && pool.length > 0) {
            const picked = (0, MathUtil_1.pickWeighted)(pool, (item) => item.weight);
            if (!picked) {
                break;
            }
        ...
        if ((context === null || context === void 0 ? void 0 : context.forceEvolutionAssist) && selected.length > 0 && !selected.some((item) => item.supportsEvolution)) {
