...
class UIManager {
    ...
    drawStartPanel(ctx, width, height, stats) {
        ...
        if (stats.activeTab === "home") {
            ...
            if (stats.selectedMode === "story") {
                ...
                const starText = `章节星级：${"★".repeat(stats.selectedStoryChapterBestStars)}${"☆".repeat(Math.max(0, 3 - stats.selectedStoryChapterBestStars))}`;
            ...
            if (stats.selectedMode === "daily") {
                ...
                ctx.fillText(stats.dailyChallengeRewardClaimed ? "今日挑战首通奖励：已领取" : "今日挑战首通奖励：可领取", cardX + 24, cardY + 218);
                ctx.fillStyle = "#b6d0ef";
                ctx.font = "16px Microsoft YaHei";
            }
            else {
            ...
            for (let i = 0; i < pageCards.length; i += 1) {
                ...
                ctx.fillText(`碎片 ${weapon.fragmentCount}  |  伤害加成 +${(weapon.damageBonus * 100).toFixed(0)}%`, cardX + 48, rowY + 56);
                ...
                const nextText = weapon.nextUpgradeCost > 0
                ...
                this.addButton({
                    ...
                    text: weapon.upgradeLevel >= weapon.maxUpgradeLevel ? "满级" : weapon.canUpgrade ? "升级" : "碎片不足",
                    x: cardX + cardW - 114,
                    y: rowY + 26,
                    w: 84,
                    h: 38,
                    color: weapon.upgradeLevel >= weapon.maxUpgradeLevel
                        ? "#5d7388"
                        : weapon.canUpgrade
                            ? "#6de6a6"
                            : "#6d7d90",
                    textColor: weapon.upgradeLevel >= weapon.maxUpgradeLevel
                        ? "#d4e0ec"
                        : weapon.canUpgrade
                            ? "#082417"
                            : "#d5e1ed"
            ...
            if (storyActive) {
                ctx.fillText(`标准章节: 第${stats.selectedStoryChapterLevel}/${stats.storyChapterCount}章 ${stats.selectedStoryChapterName}`, cardX + 24, cardY + 148);
                ...
                const chapterStarText = `星级：${"★".repeat(stats.selectedStoryChapterBestStars)}${"☆".repeat(Math.max(0, 3 - stats.selectedStoryChapterBestStars))}`;
                ...
                this.drawTruncatedText(ctx, `首通奖励：${stats.selectedStoryChapterFirstClearRewardPreview}${stats.selectedStoryChapterFirstClear ? "（已领取）" : ""}`, cardX + 24, cardY + 258, cardW - 180, 12);
                this.addButton({
                ...
                this.addButton({
                    ...
                    w: width * 0.14,
                    h: 28,
                    color: stats.selectedStoryChapterLevel < stats.storyChapterCount
                        ? nextChapterUnlocked
                            ? "#7fb5e6"
                            : "#8a7268"
                        : "#5c7088",
                    textColor: stats.selectedStoryChapterLevel < stats.storyChapterCount
                        ? nextChapterUnlocked
                            ? "#081524"
                            : "#ffe1d0"
                        : "#a7bfd8"
            ...
            ctx.fillText(`目标${this.formatTime(stats.dailyChallengeTarget)} | 奖励+${stats.dailyChallengeReward} DNA`, cardX + 24, cardY + 304);
            this.addButton({
                id: "mode_story",
    ...
    drawPausePanel(ctx, width, height, info) {
    ...
    drawEncyclopediaPanel(ctx, width, height, entries, debugMode, page, pageCount) {
    ...
    drawButton(ctx, button, fontSize = 16) {
    ...
    drawTruncatedText(ctx, text, x, y, maxWidth, fontSize) {
    ...
    drawGlassSurface(ctx, x, y, w, h, radius, accentColor) {
    ...
    drawMeter(ctx, x, y, w, h, ratio, fromColor, toColor) {
    ...
    drawSoftOrb(ctx, x, y, radius, color, alpha) {
