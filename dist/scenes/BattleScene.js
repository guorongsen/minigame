...
const CharacterMutationSystem_1 = require("../systems/CharacterMutationSystem");
const CollisionSystem_1 = require("../systems/CollisionSystem");
const DropSystem_1 = require("../systems/DropSystem");
const DailyChallengeSystem_1 = require("../systems/DailyChallengeSystem");
const EffectSystem_1 = require("../systems/EffectSystem");
const EnemySpawner_1 = require("../systems/EnemySpawner");
const EncyclopediaSystem_1 = require("../systems/EncyclopediaSystem");
const EvolutionSystem_1 = require("../systems/EvolutionSystem");
const ObstacleSystem_1 = require("../systems/ObstacleSystem");
const UpgradeSystem_1 = require("../systems/UpgradeSystem");
 ...
 */
class BattleScene {
    constructor(width, height, inputManager, ui) {
        this.width = width;
        this.height = height;
        ...
        this.enemies = [];
        this.bossHazards = [];
        this.obstacleSystem = new ObstacleSystem_1.ObstacleSystem();
        this.weaponSystem = new WeaponSystem_1.WeaponSystem();
        this.evolutionSystem = new EvolutionSystem_1.EvolutionSystem();
        this.characterMutationSystem = new CharacterMutationSystem_1.CharacterMutationSystem();
        this.upgradeSystem = new UpgradeSystem_1.UpgradeSystem();
        this.chestSystem = new ChestSystem_1.ChestSystem();
        this.dropSystem = new DropSystem_1.DropSystem();
        this.dailyChallengeSystem = new DailyChallengeSystem_1.DailyChallengeSystem();
        this.collisionSystem = new CollisionSystem_1.CollisionSystem();
        this.effectSystem = new EffectSystem_1.EffectSystem();
        this.encyclopediaSystem = new EncyclopediaSystem_1.EncyclopediaSystem();
        this.elapsedTime = 0;
        this.kills = 0;
        ...
        this.evolvedBaseIdsThisRun = new Set();
        this.pityInjectedCount = 0;
        this.pityHintShown = false;
        this.levelUpOptions = [];
        this.extraAdOption = null;
    ...
    drawPlayerHeadBars(ctx) {
        const barWidth = 74;
        const barHeight = 6;
        const x = this.player.x - barWidth * 0.5;
        const y = this.player.y - this.player.radius - 30;
        const hpRatio = (0, MathUtil_1.clamp)(this.player.hp / Math.max(1, this.player.maxHp), 0, 1);
        const expRatio = (0, MathUtil_1.clamp)(this.player.exp / Math.max(1, this.player.expToNext), 0, 1);
    ...
    drawMiniMap(ctx) {
        ...
        const toMapX = (worldX) => x + worldX * scaleX;
        const toMapY = (worldY) => y + worldY * scaleY;
        ...
        for (const obstacle of this.obstacleSystem.obstacles) {
            ...
            ctx.fillRect(toMapX(obstacle.x), toMapY(obstacle.y), Math.max(1, obstacle.w * scaleX), Math.max(1, obstacle.h * scaleY));
        }
        ...
        const camLeft = (0, MathUtil_1.clamp)(this.cameraX - visibleW * 0.5, 0, Math.max(0, this.worldWidth - visibleW));
        const camTop = (0, MathUtil_1.clamp)(this.cameraY - visibleH * 0.5, 0, Math.max(0, this.worldHeight - visibleH));
    ...
    drawBossDirectionPointer(ctx) {
        ...
        const nearRatio = (0, MathUtil_1.clamp)(1 - dist / Math.max(420, threatRange), 0, 1);
        ...
        const px = (0, MathUtil_1.clamp)(this.width * 0.5 + nx * radius + jx, margin, this.width - margin);
        const py = (0, MathUtil_1.clamp)(this.height * 0.5 + ny * radius + jy, margin + 12, this.height - margin);
    ...
    startRun(withStartBuff) {
        ...
        this.obstacleSystem.generate(this.worldWidth, this.worldHeight, this.cfg.getBalance().mapObstacleCount || 16, this.player.x, this.player.y, {
    ...
    buildChestMutationRewards(boosted) {
    ...
    triggerEvolutionFeedback(name, color) {
    ...
    drawDamageFlash(ctx) {
        ...
        const alpha = (0, MathUtil_1.clamp)(this.damageFlashTime / 0.16, 0, 1) * 0.26;
        ctx.globalAlpha = alpha;
        ctx.fillStyle = "#ff3b48";
        ctx.fillRect(0, 0, this.width, this.height);
        ctx.globalAlpha = 1;
    }
    drawEvolutionFeedback(ctx) {
        if (this.evolutionFlashTime > 0) {
            const flashAlpha = (0, MathUtil_1.clamp)(this.evolutionFlashTime / 0.28, 0, 1) * 0.33;
        ...
        const ratio = (0, MathUtil_1.clamp)(this.evolutionBannerTime / 1.1, 0, 1);
        let alpha = 1;
        if (ratio > 0.78) {
            alpha = (1 - ratio) / 0.22;
        }
        ...
        alpha = (0, MathUtil_1.clamp)(alpha, 0, 1);
    ...
    updateCamera() {
        ...
        if (this.worldWidth <= this.width) {
            ...
            this.cameraX = (0, MathUtil_1.clamp)(this.player.x, halfW, this.worldWidth - halfW);
        }
